...
async def send_new_post(c, conn, user_id, user_message: Message):
    c.execute("select tg_first_name, tg_last_name, tg_username "
              "from users where user_id = %s", (user_id,))
    user_first_name, user_last_name, user_username = c.fetchone()
    channel_id, chat_id = config.TECH_CHANNEL_ID, config.TECH_CHAT_ID
    ...
    c.execute("insert into answers_posts (post_id, client_id) values (%s, %s)",
              (new_post_id, user_id,))
    conn.commit()
